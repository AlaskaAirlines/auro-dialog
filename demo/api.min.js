...
class AuroLoader extends h {
  ...
  render() {
    return ke$1`
      ${this.defineTemplate().map((idx) => ke$1`
...
class AuroButton extends h {
  ...
  render() {
    ...
    return ke`
      <button
        part="button"
        aria-label="${to(this.arialabel ? this.arialabel : undefined)}"
        aria-labelledby="${to(this.arialabelledby ? this.arialabelledby : undefined)}"
        ?autofocus="${this.autofocus}"
        class="${Rt(classes)}"
        ?disabled="${this.disabled || this.loading}"
        ?onDark="${this.onDark}"
        title="${to(this.title ? this.title : undefined)}"
        name="${to(this.name ? this.name : undefined)}"
        type="${to(this.type ? this.type : undefined)}"
        variant="${to(this.variant ? this.variant : undefined)}"
        .value="${to(this.value ? this.value : undefined)}"
        @click="${() => {}}"
      >
        ${to(this.loading ? ke`<${this.loaderTag} pulse part="loader"></${this.loaderTag}>` : undefined)}

        <span class="contentWrapper">
          <span class="textSlot" part="text">
            ${this.iconOnly ? undefined : ke`<slot></slot>`}
          </span>
...
var tokensCss = i$2`:host{--ds-auro-icon-color: var(--ds-color-icon-primary-default, $ds-color-icon-primary-default);--ds-auro-icon-size: var(--ds-size-300, $ds-size-300)}`;
...
class ComponentBase extends h {
  ...
  render() {
    ...
    return ke`
      <div class="${Rt(classes)}" id="dialog-overlay" part="dialog-overlay" @click=${this.handleOverlayClick}></div>

      <div role="dialog" id="dialog" class="${Rt(contentClasses)}" part="dialog" aria-labelledby="dialog-header" tabindex="-1">
        ${this.unformatted
        ? ke`
