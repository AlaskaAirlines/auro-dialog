...
/**
 * Replace all instances of @aurodesignsystem, dialog, Dialog, auro and Auro accordingly
 */
...
function formatTemplateFileContents(content, destination) {
  ...
  result = result.replace(/\dialog(?!\()/g, nameExtractionData.name);
  result = result.replace(/\Dialog(?!\()/g, nameExtractionData.nameCap);
